...
aid_and_sym_validation_query = text("select exists(select aid from account where aid=:aid) and exists(select sym from stock where sym=:sym);")
...
def getHoldings():
    ...
    aid_and_sym_exists = db.session.execute(aid_and_sym_validation_query, {"aid": aid, "sym": sym}).scalar()
    shares = -1
    if aid_and_sym_exists:
        total_holdings_query = text("select coalesce(sum(case when type = 'buy' then shares else shares * -1 end), 0) as shares "
                                    "from trade T where T.sym = :sym and T.aid = :aid")
        shares = db.session.execute(total_holdings_query, {"sym": sym, "aid": aid}).scalar()
    return jsonify({'shares': float(shares)})
...
def trade():
    ...
    is_valid_trade = type in ('buy', 'sell') and shares >= 0 and price >= 0
    is_not_oversell = type == 'buy' or total_shares >= shares
    res = "fail"

    if is_valid_trade and aid_and_sym_exists and is_not_oversell:
